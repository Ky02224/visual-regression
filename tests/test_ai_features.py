...
def test_dom_diff_reflow_confirms_a_removal_the_shift_would_have_masked():
    # Removing an element reflows everything below it upward by that element's
    # own height. Those displaced siblings land in the `moved` bucket, which
    # resolves ahead of a weak (identity-less) missing verdict — so the removal
    # used to be reported as a layout shift, naming the consequence rather than
    # the cause. Two siblings agreeing on a rise equal to the gap is arithmetic,
    # not proximity, so it outranks the shift.
    baseline = [
        {"tag": "li", "x": 100, "y": 100, "w": 200, "h": 40},
        {"tag": "li", "x": 100, "y": 140, "w": 200, "h": 40, "txt": "second row here"},
        {"tag": "li", "x": 100, "y": 180, "w": 200, "h": 40, "txt": "third row here"},
    ]
    current = [
        {"tag": "li", "x": 100, "y": 100, "w": 200, "h": 40, "txt": "second row here"},
        {"tag": "li", "x": 100, "y": 140, "w": 200, "h": 40, "txt": "third row here"},
    ]
    label, evidence = diagnose_from_dom_diff(baseline, current, _region(y=90, h=140))
    assert label == "missing-element"
    assert "shifting up" in evidence


def test_dom_diff_still_reports_layout_issue_for_an_ordinary_shift():
    # One element moving on its own, with nothing vanished, must stay a layout
    # issue — the reflow rule needs a missing candidate AND agreement from two
    # elements below it, so an isolated move cannot satisfy it.
    baseline = [
        {"tag": "li", "x": 100, "y": 100, "w": 200, "h": 40, "txt": "first row here"},
        {"tag": "li", "x": 100, "y": 140, "w": 200, "h": 40, "txt": "second row here"},
    ]
    current = [
        {"tag": "li", "x": 100, "y": 100, "w": 200, "h": 40, "txt": "first row here"},
        {"tag": "li", "x": 260, "y": 140, "w": 200, "h": 40, "txt": "second row here"},
    ]
    label, _ = diagnose_from_dom_diff(baseline, current, _region(y=90, h=140))
    assert label == "layout-issue"
