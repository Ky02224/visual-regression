  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="space-y-4">
        ...
        {visibleRuns.map((run) => {
          const isItemExpanded = expandedId === run.id;
          return (
            <div 
              key={run.id} 
              onClick={() => setExpandedId(isItemExpanded ? null : run.id)}
              className="group cursor-pointer rounded-md border border-slate-100 p-4 transition hover:bg-slate-50/80 dark:border-slate-800 dark:hover:bg-slate-800/70"
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-sm font-bold text-[var(--on-surface)] group-hover:text-accent transition">{run.message}</span>
                ...
                <div className="flex items-center gap-2 text-slate-400 group-hover:text-slate-600 dark:text-slate-600 dark:group-hover:text-slate-400">
                  {isItemExpanded ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                </div>
              </div>

              {isItemExpanded && (
                <div className="mt-3 border-t border-slate-100 pt-3 dark:border-slate-850 text-xs text-[var(--on-surface-variant)] space-y-2 animate-fadeIn">
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 font-medium">
                    <div>
                      <span className="font-semibold text-[var(--on-surface)]">Activity ID:</span> <span className="font-mono">#{run.id}</span>
                    </div>
                    <div>
                      <span className="font-semibold text-[var(--on-surface)]">Execution Target:</span> <span className="font-mono">{run.branch}</span>
                    </div>
                    <div>
                      <span className="font-semibold text-[var(--on-surface)]">Outcome Status:</span>{' '}
                      <span className={cn('font-bold', run.status === 'success' ? 'text-emerald-500' : 'text-rose-500')}>
                        {run.status.toUpperCase()}
                      </span>
                    </div>
                    <div>
                      <span className="font-semibold text-[var(--on-surface)]">Timestamp:</span> {new Date(run.timestamp * 1000).toLocaleString()}
                    </div>
                  </div>
                  <div className="rounded-lg bg-slate-50 p-3 mt-2 dark:bg-slate-800/40 font-medium">
                    <span className="font-semibold text-[var(--on-surface)]">Description: </span>
                    {run.message.includes('Suite execution')
                      ? 'This entry records an automated visual regression suite execution run. The tool captured page screenshots and cross-checked them against baseline reference states.'
                      : run.message.includes('Visual check')
                      ? 'This entry represents a specific UI visual test verification. The system captured the viewport/layout and verified structural alignment.'
                      : 'This entry records an integration webhook trigger, credential lifecycle update, or CI runner event.'}
                  </div>
                </div>
              )}
            </div>
